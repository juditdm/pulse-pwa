# Fase 18 — Pulido visual y feedback

## Objetivo de esta fase

Con toda la funcionalidad de planificación ya resuelta (sesiones, favoritos, plantillas, récords, constancia, duplicar sesión, ejercicios personalizados), esta fase se centra en hacer que Pulse **se sienta** más como una app deportiva premium, sin tocar ninguna lógica de datos.

Añadimos:

- Estados vacíos más cuidados y con acción directa (por ejemplo, un botón para crear tu primera sesión, en vez de solo texto).
- Un sistema de "toast" (aviso flotante breve) para sustituir los `alert()` que interrumpen el flujo, como al añadir un set o guardar una plantilla.
- Iconos SVG ligeros en la barra inferior y en botones clave, en vez de solo texto.
- Animación breve al marcar un set como completado y al guardar la sesión activa.
- Una pantalla de carga inicial breve mientras se lee IndexedDB, para evitar el parpadeo de "0 sesiones" al abrir la app.

No tocamos:

- Ningún cálculo de récords, constancia, favoritos ni plantillas.
- La estructura de datos guardada.
- Los `alert()` que piden confirmación antes de una acción importante (por ejemplo, "faltan ejercicios para finalizar"), esos se quedan como están porque ahí sí quieres que el usuario pare a leer.
...
```javascript
const CACHE_NAME = "pulse-static-v9";
```

Si hiciera falta una corrección puntual, usaríamos `v9.1`, `v9.2`, etc.
...
Añadimos un contenedor de toasts, una pantalla de carga inicial, e iconos SVG en la barra inferior. El resto de secciones son idénticas a la fase 17.
...
```css
.boot-screen { position:fixed; inset:0; z-index:100; display:flex; align-items:center; justify-content:center; background:var(--background); transition:opacity .35s ease; }
.boot-screen.boot-hidden { opacity:0; pointer-events:none; }
.boot-mark { font-size:34px; font-weight:900; letter-spacing:-1.5px; color:var(--dark-green); animation:pulseMark 1.1s ease-in-out infinite; }
@keyframes pulseMark { 0%,100%{opacity:.55;transform:scale(.98)} 50%{opacity:1;transform:scale(1)} }

.toast-container { position:fixed; top:max(14px,env(safe-area-inset-top)); left:50%; z-index:90; display:flex; flex-direction:column; gap:8px; width:min(calc(100% - 30px),420px); transform:translateX(-50%); pointer-events:none; }
.toast { padding:12px 16px; border-radius:14px; background:var(--ink); color:#fff; font-size:13px; font-weight:700; box-shadow:var(--shadow); opacity:0; transform:translateY(-10px); transition:opacity .2s ease, transform .2s ease; }
...
.empty-state-icon { width:38px; height:38px; color:var(--muted); }
.empty-state-text { margin:0; }
.empty-state-action { padding:10px 16px; border:0; border-radius:12px; background:var(--dark-green); color:#fff; font-size:12px; font-weight:800; }

.complete-set-button.just-completed { animation:setPop .28s ease; }
@keyframes setPop { 0%{transform:scale(1)} 45%{transform:scale(1.18)} 100%{transform:scale(1)} }

.save-status.saved-flash { animation:savedFlash .5s ease; }
@keyframes savedFlash { 0%{opacity:.4} 100%{opacity:1} }
```
...
Añadimos el sistema de toasts, la pantalla de arranque, estados vacíos con acción, y pequeñas animaciones, sin tocar ningún cálculo existente. Sustituimos los `alert()` informativos (por ejemplo, "copia importada correctamente") por toasts, y dejamos los `alert()` de validación (por ejemplo, "escribe un nombre") tal cual, porque ahí interesa que el usuario pare a leerlo.
...
1. Recarga la app.
2. Confirma que aparece brevemente el mensaje "Pulse" con una animación suave antes de mostrar el Resumen, en vez de un parpadeo de "0 sesiones".

## B. Toasts

1. Marca un ejercicio como favorito y confirma que aparece un aviso flotante arriba, en vez de nada.
2. Quítalo de favoritos y confirma el aviso correspondiente.
3. Crea una plantilla y confirma el aviso de guardado.
4. Crea un ejercicio personalizado y confirma el aviso.
5. Duplica una sesión y confirma el aviso al crearla.
6. Finaliza una sesión y confirma el aviso "Sesión guardada correctamente".
7. Exporta una copia y confirma el aviso.

## C. Iconos

1. Comprueba que la barra inferior ahora muestra un icono además del texto en cada pestaña.
2. Comprueba que el icono de la pestaña activa se ve resaltado igual que antes.

## D. Estados vacíos con acción

1. Si tienes muchas sesiones, esto es difícil de probar sin borrar datos, así que puedes simplemente confirmar visualmente que el código incluye el botón de acción y omitir esta prueba en tus datos reales.
2. Alternativa segura: prueba en `http://127.0.0.1:8000` con las herramientas de desarrollo, en `Application > IndexedDB`, elimina temporalmente la base de datos `pulse_database` **solo en el navegador de pruebas del ordenador, nunca en el móvil**, recarga, y confirma que aparece el estado vacío con el botón "Crear mi primera sesión". Después vuelve a importar tu copia de seguridad para recuperar tus datos reales.

## E. Animación de set completado

1. En una sesión activa, marca un set con `✓`.
2. Confirma que el botón hace un pequeño "rebote" al completarse.

## F. Que nada se haya roto

1. Récords, constancia, favoritos, plantillas, duplicar sesión y ejercicios personalizados siguen funcionando exactamente igual.
2. Los `alert()` de validación (por ejemplo, "escribe un nombre para el ejercicio") siguen apareciendo como antes.
...
```text
Fase 18: pulido visual, toasts e iconos
```
...
Sigue exactamente los mismos pasos del documento `actualizar_pulse_en_iphone.md`, pero comprobando esta vez que el botón `+ Crear` en Ejercicios muestra el toast de confirmación y que la barra inferior tiene iconos, como señal de que el iPhone ya tiene la `v9`.

La siguiente fase nueva será la **fase 19**, con `pulse-static-v10`.
