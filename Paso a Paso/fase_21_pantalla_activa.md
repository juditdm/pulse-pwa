# Fase 21 — Mantener la pantalla activa durante el entrenamiento

## Objetivo de esta fase

Del plan original de mejoras de experiencia de uso, esta fase añade la preferencia **"Mantener pantalla activa durante el entrenamiento"**: evitar que el iPhone se bloquee solo entre set y set, para no tener que desbloquearlo constantemente en el gimnasio.

Añadimos:

- Un interruptor dentro de la pantalla **Sesión activa**.
- Al activarlo, el teléfono no bloqueará la pantalla mientras Pulse esté abierta y visible.
- Al desactivarlo, o al finalizar la sesión, se libera automáticamente para no afectar a la batería en el resto del día.
- Si el navegador o el iPhone rechazan la petición (puede ocurrir por ahorro de batería o restricciones del sistema), se avisa con un toast en vez de fallar en silencio o mostrar un error confuso.
- La preferencia se recuerda entre sesiones: si la activaste la última vez, se propone activada de nuevo la próxima vez que inicies un entrenamiento.

## Aviso importante sobre iPhone

Esta función usa la Screen Wake Lock API, disponible en iOS Safari desde la versión 16.4. Existió un fallo conocido de Apple que rompía esta función específicamente en aplicaciones instaladas en la pantalla de inicio (como tienes tú Pulse), corregido en iOS 18.4. [web:546] Por eso:

- Si tu iPhone tiene una versión de iOS anterior a 18.4 y la app instalada, es posible que el interruptor no consiga mantener la pantalla activa aunque lo actives. En ese caso verás un aviso claro, no un fallo silencioso.
- Te recomiendo probarlo primero y decirme qué tal funciona en tu iPhone concreto antes de darlo por definitivo.

No tocamos:

- Ningún cálculo de récords, constancia, gráfico, favoritos, plantillas, duplicar sesión ni la validación de copias de la fase 20.
...
```javascript
const CACHE_NAME = "pulse-static-v12";
```

Si hiciera falta una corrección puntual, usaríamos `v12.1`, `v12.2`, etc.
...
Añadimos el interruptor dentro de la cabecera de la sesión activa. El resto del archivo es idéntico a la fase 20.
...
```css
.wake-lock-row { display:flex; align-items:center; justify-content:space-between; gap:14px; margin-bottom:18px; padding:16px; border-radius:20px; }
.wake-lock-row strong { display:block; font-size:14px; }
.wake-lock-row span { display:block; margin-top:3px; font-size:11px; }
.wake-toggle { position:relative; width:50px; height:29px; padding:3px; border:0; border-radius:16px; background:#e2e7e2; transition:background .2s ease; }
.wake-toggle.active { background:var(--dark-green); }
.wake-toggle-knob { display:block; width:23px; height:23px; border-radius:50%; background:#fff; box-shadow:0 2px 5px rgba(0,0,0,.2); transition:transform .2s ease; }
.wake-toggle.active .wake-toggle-knob { transform:translateX(21px); }
```
...
Añadimos la gestión de la Screen Wake Lock API con manejo cuidadoso de errores, reactivación al volver a la app, y liberación automática al finalizar o salir de la sesión activa.
...
function exportBackup() {
    const backup = { version: "12", workouts, favorites: getFavorites(), templates: getTemplates(), customExercises: getCustomExercises() };
...
# Paso 7 — Prueba en el ordenador

1. Inicia una sesión.
2. En la parte superior de **Sesión activa**, comprueba que aparece:
...
3. Actívalo y confirma que cambia visualmente a verde.
4. Finaliza la sesión y confirma que no da ningún error.

El ordenador no bloquea la pantalla igual que un móvil, así que esta prueba es solo visual; la prueba real importante es en el iPhone.
...
```text
Fase 21: mantener pantalla activa durante el entrenamiento
```
...
# Paso 9 — Prueba real en el iPhone

Esta es la prueba que de verdad importa en esta fase.

1. Actualiza la PWA en el iPhone siguiendo el proceso habitual (abrir con conexión, cerrar del todo, reabrir).
2. Inicia una sesión de prueba.
3. Activa el interruptor **Mantener pantalla activa**.
4. Deja el iPhone quieto, sin tocarlo, durante 1-2 minutos, con Pulse abierta en primer plano.
5. Comprueba si la pantalla se bloquea sola o permanece encendida.

## Si funciona

Perfecto, ya no tendrás que desbloquear el teléfono entre sets.

## Si no funciona

Es probable que se trate del bug de Apple en PWAs instaladas que mencioné al principio. En ese caso:

1. Comprueba tu versión de iOS en **Ajustes > General > Información**.
2. Si tienes menos de iOS 18.4, dime la versión exacta y evaluamos si merece la pena mantener esta función a la espera de una actualización de Apple, o si prefieres que la simplifiquemos.

No borres ni cambies nada más hasta que me confirmes el resultado de esta prueba, porque de eso depende cómo ajustemos la fase 21.1 si hiciera falta.
