# Fase 20 — Copias de seguridad más robustas

## Objetivo de esta fase

Desde la fase 12 hemos ido añadiendo estructura a los datos (sets, favoritos, plantillas, ejercicios personalizados). La función de importar, sin embargo, sigue confiando en que el archivo tenga exactamente el formato esperado. Si alguna vez importas un archivo dañado, de otra fase muy antigua, o editado a mano por error, hasta ahora el mensaje era genérico y no protegía tus datos actuales si algo fallaba a mitad de proceso.

En esta fase añadimos:

- Validación real del archivo antes de aplicar ningún cambio: se revisa toda la estructura antes de tocar tus datos.
- Si el archivo no es válido, tus datos actuales se quedan exactamente como estaban, sin ninguna pérdida parcial.
- Un resumen antes de confirmar la importación: cuántas sesiones, favoritos y plantillas trae el archivo, para que decidas si continuar.
- Una copia de seguridad automática justo antes de importar, guardada en el propio navegador, por si quieres deshacer la importación.
- Un botón **Deshacer última importación** en Perfil, disponible solo durante la sesión actual del navegador.

No tocamos:

- Ningún cálculo de récords, constancia, gráfico, favoritos, plantillas o duplicar sesión.
- El formato de los archivos exportados, que sigue siendo compatible con todas las copias que ya tengas guardadas.
...
```javascript
const CACHE_NAME = "pulse-static-v11";
```

Si hiciera falta una corrección puntual, usaríamos `v11.1`, `v11.2`, etc.
...
Añadimos un panel de confirmación de importación y el botón de deshacer en Perfil. El resto es idéntico a la fase 19.
...
```css
.import-summary { display:grid; gap:10px; margin-bottom:6px; padding:16px; border-radius:16px; background:#f6f8f5; }
.import-summary-row { display:flex; justify-content:space-between; font-size:13px; }
.import-summary-row strong { color:var(--dark-green); }
.import-warning { margin-top:10px; padding:11px 12px; border-radius:12px; background:#fbeaea; color:var(--danger); font-size:12px; font-weight:700; }
.undo-button { border-color:var(--danger); color:var(--danger); }
```
...
Añadimos validación completa del archivo antes de tocar nada, un panel de confirmación con resumen, copia automática antes de importar, y el botón de deshacer.
...
function exportBackup() {
    const backup = { version: "11", workouts, favorites: getFavorites(), templates: getTemplates(), customExercises: getCustomExercises() };
...
## A. Importación normal

1. Ve a **Perfil**.
2. Exporta una copia.
3. Vuelve a importar esa misma copia.
4. Confirma que aparece el panel de resumen con el número correcto de sesiones, favoritos, plantillas y ejercicios personalizados.
5. Pulsa **Importar de todas formas**.
6. Confirma el toast de éxito y que tus datos siguen intactos.

## B. Archivo inválido

1. Crea un archivo de texto cualquiera con la extensión `.json` que no tenga relación con Pulse (por ejemplo, escribe `{"hola": "mundo"}` y guárdalo).
2. Intenta importarlo.
3. Confirma que aparece un aviso claro de que el archivo no es válido, y que tus sesiones **no cambian**.

## C. Deshacer importación

1. Exporta tu copia actual real (por seguridad).
2. Importa esa misma copia de nuevo, confirmando la importación.
3. Ve a **Perfil**.
4. Confirma que aparece el botón **Deshacer última importación**.
5. Púlsalo y confirma el aviso.
6. Confirma que tus datos siguen siendo correctos (en este caso no deberían cambiar visualmente, porque importaste una copia igual a la que ya tenías, pero comprueba que el proceso no genera errores).

## D. Que nada se haya roto

1. Gráfico de evolución, toasts, iconos, récords, constancia, favoritos, plantillas y duplicar sesión siguen funcionando.
...
```text
Fase 20: validar copias de seguridad y permitir deshacer
```
...
Sigue el mismo proceso habitual: abre con conexión, cierra del todo, reabre. Comprueba que al importar una copia aparece el panel de resumen antes de aplicar los cambios.

La siguiente fase nueva será la **fase 21**, con `pulse-static-v12`.
